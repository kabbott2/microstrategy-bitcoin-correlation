...
"""
fetch_mstr.py
Fetches MicroStrategy (MSTR) price and volume data from Yahoo Finance.
Data starts August 11, 2020 - when MSTR began its Bitcoin acquisition strategy.
"""
...
# MSTR's first BTC purchase announcement date
START_DATE = "2020-08-11"
TICKER = "MSTR"


def fetch_mstr_data(end_date: str = None) -> pd.DataFrame:
    """
    Fetch MSTR historical data and compute derived metrics.
    ...
    mstr = yf.Ticker(TICKER)
    df = mstr.history(start=START_DATE, end=end_date)
...
if __name__ == "__main__":
    # Quick test
    data = fetch_mstr_data()
    print(f"Fetched {len(data)} trading days of MSTR data\n")
    print(data.tail())
    print("\nSummary:")
